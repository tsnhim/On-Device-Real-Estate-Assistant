...
INPUT_JSON = ROOT.parent.parent / "results" / "all_benchmarks.json"
...
def load_rows() -> list[dict]:
    ...
    rows = []
    baseline = next(
        (r for r in records if r["model_name"].lower() in {"baseline", "flan_t5_zillow_final1_baseline"}),
        max(records, key=lambda r: float(r["efficiency"]["disk_size_mb"])),
    )
...
def classify_family(name: str) -> str:
    ...
    if normalized in {"baseline", "flan_t5_zillow_final1_baseline"}:
        return "Baseline"
    has_quant = any(term in normalized for term in ["int8", "fp16", "bf16"])
    has_prune = "prune" in normalized
...
def write_accuracy_and_retention(rows: list[dict], manifest: list[dict]) -> None:
    ...
    body = [
        '<text class="subtitle" x="36" y="55">Shows actual Token F1 and the same score as percentage retained versus the selected reference model.</text>',
        f'<text class="label" x="{left}" y="70">Token F1</text>',
        f'<text class="label" x="{left + f1_w + 112}" y="70">Retention vs reference</text>',
    ]
    ...
    path.write_text(svg_page(width, height, "Accuracy: Token F1 and Reference Retention", "\n".join(body)), encoding="utf-8")
    manifest.append({"file": path.name, "title": "Accuracy: Token F1 and reference retention"})
...
def write_retention_size_scatter(rows: list[dict], manifest: list[dict]) -> None:
    ...
    body.append(f'<text class="label" x="{left + plot_w / 2 - 88:.1f}" y="{height - 28}">Disk size reduction vs reference</text>')
    body.append(f'<text class="label" transform="translate(24 {top + plot_h / 2 + 76:.1f}) rotate(-90)">Token F1 retention vs reference</text>')
...
def write_pruning_structure_comparison(rows: list[dict], manifest: list[dict]) -> None:
    prune_rows = [
        ...
        and ("structured" in row["model_name"].lower() or "unstructured" in row["model_name"].lower())
    ]
    data = sorted(
        prune_rows,
        key=lambda row: (
            0 if row["model_name"].lower().startswith("unstructured") else 1,
            -row["token_f1"],
        ),
    )
    if not data:
    ...
    for row_index, row in enumerate(data):
        ...
        is_structured = row["model_name"].lower().startswith("structured")
...
def write_gated_balanced_ranking(scored: list[dict], manifest: list[dict]) -> None:
    ...
    bar_w = 390
    body = [
        f'<text class="subtitle" x="36" y="55">Accuracy gate: Token F1 retention must be at least {ACCURACY_GATE_RETENTION:.0f}% of the selected reference. Passing models are ranked by efficiency: latency 40%, throughput 35%, disk size 25%.</text>',
...
<body>
  ...
  <p>Generated from <code>results/all_benchmarks.json</code>.</p>
  {''.join(cards)}
</body>
</html>
"""
